...
struct SlidingWindow {
  ...
  SmallVector<AffineMap> preloadMaps;
};
...
/// Build the map used to preload one window element at the first iteration.
/// The caller has checked full composition with the original access map.
static AffineMap getPreloadMap(AffineForOp loop, int64_t offset) {
  AffineMap lowerBoundMap = loop.getLowerBoundMap();
  AffineExpr index = lowerBoundMap.getResult(0) + offset;
  return AffineMap::get(lowerBoundMap.getNumDims(),
  ...
  for (auto &[source, loads] : loadsBySource) {
    ...
    for (size_t windowBegin = 0; windowBegin < loads.size();) {
      ...
      if (windowEnd - windowBegin >= 2) {
        ...
        SmallVector<AffineMap> preloadMaps;
        for (const LoadAtOffset &access : llvm::drop_end(windowLoads)) {
          if (!isFirstIterationLoadCompositionRepresentable(access.load, loop,
                                                            accessExpander))
            break;
          preloadMaps.push_back(getPreloadMap(loop, access.offset));
        }
        if (preloadMaps.size() == windowLoads.size() - 1)
          return SlidingWindow{source, std::move(windowLoads),
                               std::move(preloadMaps)};
      }
...
/// Materialize the first window before the loop, append its element state to
...
static FailureOr<AffineForOp> rewriteSlidingWindow(IRRewriter &rewriter,
  ...
  for (auto [access, preloadMap] :
       llvm::zip_equal(llvm::drop_end(window.loads), window.preloadMaps)) {
    auto preload =
