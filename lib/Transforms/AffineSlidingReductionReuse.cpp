...
/// Return the first legal and potentially profitable window in block order.
static std::optional<ReductionWindow>
findReductionWindow(AffineForOp loop, AliasAnalysis &aliasAnalysis) {
  ...
  for (Operation &operation : loop.getBody()->without_terminator()) {
    if (!getReductionKind(&operation))
      continue;
    // A single-use fold feeding another fold of the same kind is an internal
    // node of a larger candidate.
    if (operation.hasOneUse()) {
      Operation *user = *operation.getUsers().begin();
      if (user->getName() == operation.getName() &&
          user->getBlock() == operation.getBlock())
        continue;
    }
    std::optional<ReductionWindow> window =
        matchReductionTree(&operation, loop, accessExpander);
    if (!window || findModifyingOperation(loop, window->source, aliasAnalysis))
      continue;
    return window;
  }
  return std::nullopt;
