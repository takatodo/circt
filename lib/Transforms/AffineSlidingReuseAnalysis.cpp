//===- AffineSlidingReuseAnalysis.cpp - Sliding reuse analysis -*- C++ -*-===//
//
...
bool isFirstIterationLoadCompositionRepresentable(
    AffineLoadOp load, AffineForOp loop, CheckedLinearExprExpander &expander) {
  AffineMap lowerMap = loop.getLowerBoundMap();
  if (lowerMap.getNumResults() != 1)
    return false;
  std::optional<CheckedLinearExpr> lower =
      expander.expand(lowerMap.getResult(0), loop.getLowerBoundOperands(),
                      lowerMap.getNumDims());
  if (!lower)
    return false;

  AffineMap accessMap = load.getAffineMap();
  for (AffineExpr result : accessMap.getResults()) {
    std::optional<CheckedLinearExpr> access =
        expander.expand(result, load.getMapOperands(), accessMap.getNumDims());
    if (!access)
      return false;
    std::optional<CheckedLinearExpr> substitutedLower =
        scaleCheckedLinearExpr(*lower, access->inductionCoefficient);
    if (!substitutedLower)
      return false;
    access->inductionCoefficient = 0;
    if (!addCheckedLinearExpr(*substitutedLower, *access))
      return false;
  }
  return true;
}
...
bool mayModifySource(AffineForOp loop, Value source,
