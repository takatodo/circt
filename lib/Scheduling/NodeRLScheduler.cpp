...
scheduling::exploreNodeRLPareto(SharedOperatorsProblem &prob, Operation *lastOp,
  ...
  if (allocations.empty())
    return prob.getContainingOp()->emitError(
        "node-rl Pareto exploration requires at least one allocation");

  SmallVector<ResourceParetoPoint> evaluated;
  evaluated.reserve(allocations.size());
  for (const auto &allocation : allocations) {
    DenseSet<Problem::ResourceType> seen;
    for (const auto &entry : allocation.limits) {
      ...
      if (!seen.insert(entry.resource).second)
        return prob.getContainingOp()->emitError()
    ...
    if (seen.size() != prob.getResourceTypes().size())
      return prob.getContainingOp()->emitError(
          "resource allocation must specify every resource type exactly once");

    if (failed(scheduleNodeRL(prob, lastOp, options)) || failed(prob.verify()))
      return failure();
    evaluated.push_back(
        {allocation, *prob.getStartTime(lastOp), costFunction(allocation)});
  }
  ...
  for (const auto &entry : selected->allocation.limits)
    prob.setLimit(entry.resource, entry.limit);
  if (failed(scheduleNodeRL(prob, lastOp, options)) || failed(prob.verify()))
    return failure();
  return frontier;
...
scheduling::exploreNodeRLPareto(ModuloProblem &prob, Operation *lastOp,
  ...
  for (const auto &allocation : allocations) {
    ...
    evaluated.push_back({allocation, *prob.getStartTime(lastOp),
                         costFunction(allocation),
                         *prob.getInitiationInterval()});
  }
